...
class Env(object):
    ...
    def step(self, action, total_fuel, dis, data, TTC_violate, fTTC_total):
        ...
        # fJerk = -2*(np.count_nonzero(self.SimJerkData[:self.timeStep-1] > 1.5))/(self.timeStep-1)
        ...
        if space<0:
            reward = fFuel +fAcc + fJerk+fTL +fHdw+fCol+ fIDM#
            # reward = -1
        else:
            reward =fFuel +fAcc + fJerk+fTL +fHdw+ fTTC + fIDM#
        #reward = fFuel+ fHdw
        # record reward info
