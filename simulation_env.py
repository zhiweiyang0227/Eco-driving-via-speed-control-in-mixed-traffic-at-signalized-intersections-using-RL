# -*- coding: utf-8 -*-
"""
Created on Tue May  3 12:22:28 2022
...
class Env(object):
    ...
    def step(self, action, total_fuel, dis, data, TTC_violate, fTTC_total):
        def fuel_model(v, a):
        ...
        if dis+data[0,7]<=470:
            ...
            for i in range(4):
                v_limit_TL.append(((470-data[0,7]-dis))/(fut_TL[i]-time))
        ...
        #fAcc = - (action**2)/4**2
        fAcc = -1*(abs(action)/4)**(1/2)
        self.lastAction = action

        if self.TTC >= 0 and self.TTC <= self.TTC_threshold:
            TTC_violate+=1
            fTTC = 1*((self.TTC/self.TTC_threshold)**2-1)
            fTTC_total += 1*((self.TTC/self.TTC_threshold)**2-1)
        ...
        fHdw =1*(dis / (data[self.timeStep-1,5]-data[0,5]))**2/(50/3.6)**2 #(dis / (data[self.timeStep-1,5]-data[0,5]))/(50/3.6)#
        try:
            fFuel = (math.log(dis/(total_fuel*1000)+1,25))**5
        except:
            print('dis,total_fuel', dis,total_fuel)
        # calculate the reward
        if space<0:
            reward =  fTL +fCol +fFuel+ fHdw#fJerk+ fAcc +
            # reward = -1
        else:
            reward =  fTL +fTTC ++ fFuel + fHdw#fJerk+ fAcc
        #reward = fFuel+ fHdw
        # record reward info
        
        rewardInfo = [self.TTC, hdw, jerk, fTTC, fHdw, fJerk, fAcc, total_fuel, fFuel,fTL,dis,fCol,TTC_violate,fTTC_total]
        ...
        if self.timeStep == self.TimeLen or self.isCollision == 1:
            ...
            a_bound_jerk = [action-step_duration_next*10,action+step_duration_next*10] 
