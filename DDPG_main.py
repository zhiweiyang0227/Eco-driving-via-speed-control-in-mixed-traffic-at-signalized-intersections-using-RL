# -*- coding: utf-8 -*-
"""
Created on Tue May  3 12:14:05 2022
...
for run in [base_name]:
    ...
    for i in range(total_episode):
        ...
        score = 0
        score_s, score_e, score_c, score_a, score_tl, score_col = 0, 0, 0, 0, 0, 0  # part objective scores
        ori_loc = data[0,7]
        dis = 0
        total_fuel = 0
        fTTC_total =0
        ...
        while True:
            ...
            if done:
                ...
                if score_col==-env.penalty:#env.isCollision == 1:
                    score = score_tl+score_col+score_f +score_e#score_s +score_c+ score_a +
                    # score = -1
                    collision_train += 1
                else:
                    score = score_tl+score_s + + score_f +score_e#+score_col+score_c+ score_a
        ...
        if (i+1)%100==0:
            ...
            plt.show()
score_summary = pd.DataFrame(np.vstack([episode_score, rolling_score, score_fuel, score_safe, score_efficiency, score_comfort, score_action, score_SPaT, score_collision]).T)
score_summary.columns=['episode_score', 'rolling_score', 'score_fuel', 'score_safe', 'score_efficiency', 'score_comfort', 'score_action', 'score_SPaT', 'score_collision']
