# -*- coding: utf-8 -*-
"""
Created on Mon Oct 10 14:16:09 2022
...
class PolicyNetwork(nn.Module):
    ...
    # def sample(self, state, invalid_action_masks=None):
    def sample(self, state):
        mean, log_std = self.forward(state)
        # split_logits = torch.split(mean, [1])
...
class SAC_Agent:
    def __init__(self):
        ...
        self.gamma          = 0.99#0.98
        self.batch_size     = 512#200
        self.buffer_limit   = 300000#500000
    ...
    def choose_action(self, s):
        with torch.no_grad():
            action, log_prob = self.PI.sample(s.to(self.DEVICE))#, invalid_action_masks=None , _, probs
        return action, log_prob
...
for run in [base_name]:
    ...
    for i in range(EPISODE): 
        ...
        while True:
            ...
            space_pre = data[env.timeStep-1, 7]+data[env.timeStep-1, 0]-data[0,7]-dis#self.currentState[0]-relSpd*step_duration
            speed_pre = s[1]
            LVSpd_pre = env.LVSpdData[env.timeStep-1]
            # if env.timeStep-1-1/0.04>=0:
            #     LVSpd_pre_1s = env.LVSpdData[int(env.timeStep-1-1.5/0.04)]
            # else:
            #     LVSpd_pre_1s = env.LVSpdData[0]
            acc_avg_duration = 1
            acc_avg_pre = np.mean(env.LVAccData[max(0,int(env.timeStep-1-acc_avg_duration/0.04)):env.timeStep])
            delta_spd = speed_pre-LVSpd_pre
            step_duration = (data[env.timeStep,5]-data[env.timeStep-1,5])
            acc_IDM, v_IDM = IDM(space_pre, speed_pre, delta_spd, step_duration, IVT, s_s)
            curr_TL,_ = env.TL(data[env.timeStep-1,5])
            if 440<=dis+data[0,7]<470 and data[env.timeStep-1,7]+env.currentState[0]>470:
                if curr_TL is not 'green':
                    acc_IDM, v_IDM=IDM(470-(dis+data[0,7]), speed_pre, speed_pre, step_duration,IVT_TL,s_TL)             
            del_a = 0
            # if Hybrid:
                # if env.TTC<=TTC_threshold+1 and acc_IDM<a:
                #     del_a = acc_IDM-a
                #     a = acc_IDM
                # if curr_TL is 'green':
                #     if dis+data[0,7]<470 and data[env.timeStep-1,7]+env.currentState[0]>470:
                #         if acc_IDM>a:
                #             del_a = acc_IDM-a
                #             a = acc_IDM

                #     else: 
                #         # if LVSpd_pre_1s<LVSpd_pre and LVSpd_pre>speed_pre and acc_IDM>a:
                #         if acc_avg_pre>0 and LVSpd_pre>speed_pre and acc_IDM>a:
                #             del_a = acc_IDM-a
                #             a = acc_IDM
            
            # CA and traffic light violation avoidance
            state = env.s
            ...
            if a_TL is not None and a_col is not None:
                a_C_TL = max(-a_bound,min(a_TL, a_col))
                del_a += a_C_TL-a
                a = a_C_TL
            elif a_TL is None and a_col is not None:
                a_C_TL = max(-a_bound,a_col)
                del_a += a_C_TL-a
                a = a_C_TL
            elif a_TL is not None and a_col is None:
                a_C_TL = max(-a_bound,a_TL)
                del_a += a_C_TL-a
                a = a_C_TL
            else:
                del_a += 0
            ...
            s_, r, done, r_info, a_bound_jerk, a_bound_v = env.step(a, total_fuel, dis,data,TTC_violate, fTTC_total, del_a)#
            ...
            if done:
                ...
                if score_col==-env.penalty:#env.isCollision == 1:
                    score = score_c+score_a +score_f +score_tl+score_e+score_col#score_s ++score_IDM
                    # score = -1
                    collision_train += 1
                else:
                    score = score_c+score_a +score_f +score_tl+score_e+score_s#+score_col +score_IDM
        ...
        if rolling_score[i] > max_rolling_score:
            ...
            torch.save(agent.PI.state_dict(), "save_HybridByRules/sac_actor_"+str(i)+".pt")
...
writer = pd.ExcelWriter('save_HybridByRules/RL_bestTrain/score_data_training{}.xlsx'.format(random_no))		# 写入Excel文件
